...
class Loader {
	...
	var $_ob_level;
	var $_view_path		= '';
	var $_library_paths	= array();
	var $_model_paths	= array();
	var $_helper_paths	= array();
	...
	function remove_package_path($path = '', $remove_config_path = TRUE)
	{
		...
		$this->_library_paths = array_unique(array_merge($this->_library_paths, array(APPPATH, BASEPATH)));
		$this->_helper_paths = array_unique(array_merge($this->_helper_paths, array(APPPATH, BASEPATH)));
		$this->_model_paths = array_unique(array_merge($this->_model_paths, array(APPPATH)));
	...
	function _load($_data)
	{
		...
		if ($_path == '')
		{
			...
			$_file = ($_ext == '') ? $_view.EXT : $_view;
			$_path = $this->_view_path.$_file;
		}
		else
		{
			$_x = explode('/', $_path);
	 ...
	 * @return	void
	 */
	function _load_class($class, $params = NULL, $object_name = NULL)
	{
	 ...
	 * @return	null
	 */
	function _init_class($class, $prefix = '', $config = FALSE, $object_name = NULL)
	{
	 ...
	 * @return	bool
	 */
	function &_get_component($component)
	{
		$CI =& get_instance();
		return $CI->$component;
	}
	 ...
	 * @return	array
	 */
	function _prep_filename($filename, $extension)
	{
